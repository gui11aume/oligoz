...
class DNAdseq(DNAseq):
    ...
    # Complementary base-pairing.
    bp = str.maketrans(
        {k: v for (k, v) in zip("gatcrymkbvdhGATCRYMKBVDH", "ctagyrkmvbhdCTAGYRKMVBHD")}
    )
...
class RNAdseq(RNAseq):
    ...
    # Complementary base-pairing.
    bp = str.maketrans(
        {k: v for (k, v) in zip("gaucrymkbvdhGAUCRYMKBVDH", "cuagyrkmvbhdCUAGYRKMVBHD")}
    )
...
class Oligo:
    ...
    def __getattr__(self, name):
        ...
        if name == "thermoParams":
            H = Oligo.nnConst[self.seq[0]]["H"] + Oligo.nnConst[self.seq[-1:]]["H"]
            S = Oligo.nnConst[self.seq[0]]["S"] + Oligo.nnConst[self.seq[-1:]]["S"]
...
class OligoSol:
    ...
    def OwczarzyCorrection(self):
        ...
        if mono == 0 or sqrt(di) > 6 * mono:
            ...
            return (
                OligoSol.saltP["a"]
                + OligoSol.saltP["b"] * log(di)
                + f_GC * (OligoSol.saltP["c"] + OligoSol.saltP["d"] * log(di))
                + (
                    OligoSol.saltP["e"]
                    + OligoSol.saltP["f"] * log(di)
                    + OligoSol.saltP["g"] * log(di)
                )
                ** 2
                / (2 * (size - 1))
            )
        ...
        if sqrt(di) > 0.22 * mono:
            ...
            updated_d = 1.42e-5 * (
                1.279 - 4.03e-3 * log(mono) - 8.03e-3 * log(mono) ** 2
            )
            updated_g = 8.31e-5 * (0.486 - 0.258 * log(mono) + 5.25e-3 * log(mono) ** 3)
        ...
        return (4.29 * f_GC - 3.95) * 1e-5 * log(mono) + 9.40e-6 * (log(mono)) ** 2
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="extra nucleotides on left primer",
    )
    ...
    # Search options.
    parser.add_argument(
        "--qPCR", dest="qPCR", action="store_true", help="search primers for qPCR"
    )

    parser.add_argument(
        "--force", dest="force", action="store_true", help="force find a primer pair"
    )
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="allows d nucleotides shift",
    )
    ...
    parser.add_argument(
        ...
        help="oligo concentration (%1.f uM)" % (1000000 * STD_HYBCOND["conc"]),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=argparse.SUPPRESS,
        help="concentration of divalent cations (%.1f mM)" % (1000 * STD_HYBCOND["di"]),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=argparse.SUPPRESS,
        help="maximum oligo length (%d)" % STD_RULES["max_length"],
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=argparse.SUPPRESS,
        help="minimum oligo solution Tm (%d)" % (STD_RULES["min_Tm"] + ABS0),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=argparse.SUPPRESS,
        help="maximum oligo solution Tm (%d)" % (STD_RULES["max_Tm"] + ABS0),
    )

    parser.add_argument(
        "--max_self_anneal",
        metavar="M",
        dest="max_self_anneal",
        default=argparse.SUPPRESS,
        type=int,
        help="maximum self annealing (%d)" % STD_RULES["max_self_anneal"],
    )
    ...
    parser.add_argument(
        ...
        default=argparse.SUPPRESS,
        type=int,
        help="longest unspecific annealing (%d)" % STD_RULES["max_lanneal"],
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=argparse.SUPPRESS,
        help="maximum difference between Tm (%d)" % STD_RULES["max_delta_Tm"],
    )

    parser.add_argument(
        "--max_het_anneal",
        metavar="T",
        dest="max_hetero_anneal",
        default=argparse.SUPPRESS,
        type=int,
        help="maximum hetero annealing (%d)" % STD_RULES["max_hetero_anneal"],
    )

    args = parser.parse_args()

    # Post-process arguments.
    if hasattr(args, "conc"):
        args.conc /= 1000000
    if hasattr(args, "mono"):
        args.mono /= 1000
    if hasattr(args, "di"):
        args.di /= 1000
    if hasattr(args, "max_Tm"):
        args.max_Tm -= ABS0
    if hasattr(args, "min_Tm"):
        args.min_Tm -= ABS0
...
def show_primers(dict_of_HitList):
    ...
    for title, hits in dict_of_HitList.items():
        ...
        if not hits and not args.force:
            continue
